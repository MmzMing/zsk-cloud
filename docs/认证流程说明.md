...
## 0. 认证总流程 (Authentication Flow)

本系统采用 **滑块验证码 + 邮箱验证码 + RSA 加密传输 + BCrypt 密码存储** 的多重安全认证机制。
...
sequenceDiagram
    ...
    Auth->>Redis: 比对 X 坐标
    Redis-->>Auth: 校验通过
    Auth->>Redis: 生成 verifyToken (Key: captcha_verified:{token})
...
| **安全框架** | Spring Security + OAuth2 | 认证授权核心框架 |
...
| **加密算法** | RSA + BCrypt | 双重加密机制（传输层 RSA，存储层 BCrypt） |
...
```xml
    <dependencies>
        ...
        <!-- ZSK Common Security (封装 Spring Security) -->
        <dependency>
            <groupId>com.zsk.cloud</groupId>
            <artifactId>zsk-common-security</artifactId>
        </dependency>
        <!-- 包含以下核心依赖：
        <dependency>
            <groupId>org.springframework.boot</groupId>
        ...
        <dependency>
            <groupId>com.zsk.cloud</groupId>
            <artifactId>zsk-api-system</artifactId>
        </dependency>
...
// 限制每个邮箱每5分钟最多发送5次验证码，需要配置sentinel
