...
#### 接口信息
- **URL**: `GET /api/document/docNoteComment/comments/{noteId}`
- **功能**: 获取笔记的评论列表，支持分页和排序。采用B站式评论结构：所有回复统一挂在根评论下，不存在层级嵌套。评论点赞数从Redis实时获取。
...
#### 接口信息
- **URL**: `POST /api/document/docNoteComment/comment`
